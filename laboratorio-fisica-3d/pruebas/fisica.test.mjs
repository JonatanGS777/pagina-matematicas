...
// 1. Caida libre sin arrastre: debe coincidir con sqrt(2h/g)
const cl = new F.CaidaLibre({ altura: 20 });
let pasos = 0;
while (!cl.enSuelo && pasos < 100000) { cl.paso(0.0005); pasos++; }
check('caida libre tiempo', cl.t, cl.tiempoTeorico(), 0.002);
...
check('plano angulo critico', pi.anguloCriticoGrados(), Math.atan(0.25) * 180 / Math.PI, 1e-9);

// 3. Proyectil sin arrastre vs alcance teorico
...
// 4. Colision elastica masas iguales: intercambio de velocidades
const co = new F.Colision({ masaA: 1, masaB: 1, velocidadA: 3, velocidadB: 0, restitucion: 1 });
while (!co.choco) co.paso(0.001);
check('colision elastica vA', co.vA, 0, 1e-9);
check('colision elastica vB', co.vB, 3, 1e-9);
check('colision momento conservado', co.momentoTotal(), co.momentoInicial, 1e-9);
check('colision energia conservada', co.energiaCinetica(), co.energiaInicial, 1e-9);

// Colision perfectamente inelastica: misma velocidad final, momento conservado
const ci = new F.Colision({ masaA: 2, masaB: 1, velocidadA: 3, velocidadB: -1, restitucion: 0 });
while (!ci.choco) ci.paso(0.001);
check('inelastica vA=vB', ci.vA, ci.vB, 1e-9);
check('inelastica momento', ci.momentoTotal(), ci.momentoInicial, 1e-9);
check('inelastica v final', ci.vA, (2*3 + 1*-1)/3, 1e-9);

// 5. Pendulo: periodo medido vs corregido con angulo grande
...
check('pendulo periodo real vs corregido', pe.periodoMedido, pe.periodoCorregido(), 0.01);
console.log(`     (periodo pequeno seria ${pe.periodoPequeno().toFixed(4)}, real ${pe.periodoMedido.toFixed(4)})`);
// Energia se conserva sin amortiguamiento
const pe2 = new F.Pendulo({ largo: 1.5, anguloInicialGrados: 40 });
const e0 = pe2.energiaTotal();
for (let i = 0; i < 50000; i++) pe2.paso(0.0002);
check('pendulo energia conservada', pe2.energiaTotal(), e0, 1e-6);

// 6. Campo electrico: carga puntual  E = kq/r^2
...
// 7. Lorentz: radio de giro y periodo ciclotronico
...
const p = lo.posicion();
check('lorentz cierra la orbita x', p[0], 0, 1e-4);
check('lorentz cierra la orbita z', p[2], 0, 1e-4);
check('lorentz radio teorico', rTeo, 1e-6 * 2 / (1e-6 * 0.5), 1e-12);
// El radio medido: distancia maxima al centro de giro debe ser 2r
...
check('lorentz diametro medido', maxDist, 2 * rTeo, 1e-3);
// Rapidez constante (la fuerza magnetica no hace trabajo)
