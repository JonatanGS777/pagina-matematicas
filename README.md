...
| `lab/modulos/geometrico.html` | Sala de Máquinas *(shared system for the 4 `lab/modulos/` pages, each keeping its own accent — this one violet)* | Industrial control-room / instrument-panel reskin of the 3D solid-geometry calculator (box, cylinder, sphere, pyramid) — dark riveted panel chrome, an oscilloscope-style screen around the canvas, and gauge-readout results in DM Mono with a glowing teal digit shadow. Canvas outline/dashed-edge stroke colors brightened for the dark background; Font Awesome converted to inline Lucide SVG including the animate button's spinner and play-icon reset states. Fixed a dead `modelado.html` link (logo + back button pointed at a file that doesn't exist anywhere in `lab/modulos/`) to `../experimentos.html`. All volume/area/diagonal math and the slider-driven animation loop untouched |
...
and `geometria.js`), `lab/juegos.html`, and all 4 `lab/modulos/` pages
(`geometrico.html`, `datos.html`, `fisica.html`, `optimizacion.html`).
